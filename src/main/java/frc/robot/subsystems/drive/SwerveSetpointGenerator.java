...
public class SwerveSetpointGenerator {
    ...
    protected double findSteeringMaxS(double x_0, double y_0, double f_0, double x_1, double y_1, double f_1, double max_deviation, int max_iterations) {
        ...
        Function2d func = (x,y) -> {
            return LobstahMath.unwrapAngle(f_0, Math.atan2(y, x)) - offset;
        };
    ...
    public SwerveSetpoint generateSetpoint(SwerveKinematicLimits limits, final SwerveSetpoint prevSetpoint, ChassisSpeeds desiredState, double dt) {
        ...
        if (epsilonEquals(LobstahMath.toTwist2d(desiredState), new Twist2d())) {
            need_to_steer = false;
        ...
        if (all_modules_should_flip &&
            !epsilonEquals(LobstahMath.toTwist2d(prevSetpoint.chassisSpeeds), new Twist2d()) &&
            !epsilonEquals(LobstahMath.toTwist2d(desiredState), new Twist2d())) {
